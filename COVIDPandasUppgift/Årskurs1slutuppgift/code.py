...
while run == True: #While true loop
    ...
    if q1 == "NDD": #If the answer is NDD it asks another question for which kind of chart of the 2 that you can choose between
        ...
        if q1_3 == "PC": #Making a pie chart
            ...
            def autopctFunction(pct): #Function for adding procentage of the pies and also cutting out those smaller then 5%
                return ('%.2f %%' % pct) if pct > 5 else ''
            plt.pie(nt.Total_Deaths, labels = labels, autopct=autopctFunction)
            plt.show()
        if q1_3 == "BC": #Subplot Barchart
        ...
        if q1_4 == "PC": #Woooo another piechart but this time with plotly express, i dont think theres much else to explain
